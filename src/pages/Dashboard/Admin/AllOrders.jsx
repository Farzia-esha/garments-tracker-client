...
const AllOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res = await fetch(`https://garments-tracker-system.vercel.app/bookings`);
  ...
  const updateOrderStatus = async (order) => {
    ...
    try {
      const res = await fetch(`https://garments-tracker-system.vercel.app/bookings/${order._id}`, {
        method: 'PATCH',
              ...
              {filteredOrders.map(order => (
                <tr key={order._id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-center flex justify-center gap-2">
                    <button onClick={() => viewOrderDetails(order)} title="View" className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition">
                      <Eye size={20} />
                    </button>
                    <button onClick={() => updateOrderStatus(order)} title="Edit" className="p-2 bg-yellow-100 text-yellow-600 rounded-lg hover:bg-yellow-200 transition">
                      <Edit size={20} />
                    </button>
                    <button onClick={() => deleteOrder(order)} title="Delete" className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition">
                      <Trash2 size={20} />
                    </button>
                  </td>
                </tr>
